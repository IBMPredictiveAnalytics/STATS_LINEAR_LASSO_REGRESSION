...
"""Initialize the llr wrapper"""
init_wrapper("llr", os.path.join(os.path.dirname(__file__), "LLR-properties.json"))
...
def process_trace(x, y, swt, alphas, t_indices, result):
    ...
    trace = Lasso(alpha=alphas, fit_intercept=intercept, max_iter=100000)
...
def process_cv(x, y, swt, nfolds, state, alphas, t_indices, h_indices, result):
    ...
    for alpha in alphas:
        ...
        for fold_n, (train_index, val_index) in enumerate(folds.split(x, y, swt)):
            ...
            model = Lasso(alpha=alpha, fit_intercept=intercept, max_iter=100000)
